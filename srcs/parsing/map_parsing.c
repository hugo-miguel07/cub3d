...
/*   Updated: 2026/05/21 18:18:47 by antabord         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	checking_chars_n_walls(s_file *file, char **map)
{
	...
	if (file->player != 1)
		return (exit_check(INVALID_PLAYER_SPAWN, file), 0);
	return (1);
}

static int	flood_rest(s_file *file, char **map)
{
	int	y;
	int	x;
...
int	map_cpy(s_file *file)
{
	...
	map_cpy[i] = NULL;
	if (!player_pos(file, map_cpy) || !flood_rest(file, map_cpy) || free_arr(map_cpy, rows))
		return (0);
	return (1);
}
